...
<head>
    ...
    <title>家居网购~</title>
    <!-- 移动端适配 -->
    <base href="http://localhost:8080/jiaju/">
                    ...
                    <div class="header-actions">
                        <div class="header_account_list">
                            ...
                            <div class="dropdown_search">
                                <form class="action-form" action="CustomerServlet">
                                    <input type="hidden" name="action" value="getPageByName">
                                    <input class="form-control" placeholder="请输入关键词" type="text" name="name">
                        ...
                        <c:if test="${sessionScope.name!=null}">
                            <div class="header-bottom-set dropdown">
                                <a>欢迎${sessionScope.name}回来</a>
                            </div>
                            <div class="header-bottom-set dropdown">
                                <a href="memberServlet?action=Exist">退出登陆</a>
                            </div>
                            <div class="header-bottom-set dropdown">
                        ...
                        <div class="header-bottom-set dropdown">
                            <a href="views/manageLogin/manage_login.html">管理员登陆</a>
                        </div>


                        <!-- Single Wedge End -->
                        <a href="views/member/cart.jsp"
                           class="header-action-btn header-action-btn-cart pr-0" id="jump">
                            <i class="icon-handbag"> 购物车</i>
                            <span class="header-action-num" id="cart_count">${sessionScope.cart.totalCount}</span>
                        </a>
                                    ...
                                    <div class="product">
                                        <div class="thumb">
                                            ...
                                            <c:if test="${item.stock>0}">
                                                <button title="Add To Cart~" class=" add-to-cart" furnId="${item.id}" stock="${item.stock}">Add
                                                    To Cart
                                                </button>
                                            </c:if>
                                            <c:if test="${item.stock<=0}">
                                                <button title="Add To Cart~" class=" add-to-cart" furnId="${item.id}" stock="${item.stock}">
                                                    已售罄
                                                </button>
                                            </c:if>
                                        </div>
                                        <div class="content">
                                            ...
                                            <span class="price">
                                                <span class="new">厂商:　${item.maker}</span>
                                            </span>
                                            <span class="price">
...
<div class="pro-pagination-style text-center mb-md-30px mb-lm-30px mt-6" data-aos="fade-up">
    <ul>
        <%--            <li><a href="#">首页</a></li>--%>
        <%--            <li><a href="manage/furnServlet?action=getPage&begin=${requestScope.page.pageNo-1}&PageSize=3">上页</a></li>--%>
        <c:if test="${requestScope.page.pageNo>0}">
            <li><a href="${requestScope.page.url}&begin=${requestScope.page.pageNo-1}&PageSize=${requestScope.page.pageSize}">上页</a></li>
        </c:if>
        ...
        <c:if test="${requestScope.page.pageNo<=2}">
            <c:if test="${requestScope.page.totalPage<5}">
                <c:forEach begin="0" end="${requestScope.page.totalPage-1}" var="i">
                    <c:if test="${i==requestScope.page.pageNo}">
                        <li><a class="active" href="${requestScope.page.url}&begin=${i}&PageSize=${requestScope.page.pageSize}">${i}</a></li>
                    </c:if>
                    <c:if test="${i!=requestScope.page.pageNo}">
                        <li><a href="${requestScope.page.url}&begin=${i}&PageSize=${requestScope.page.pageSize}">${i}</a></li>
                    </c:if>
                </c:forEach>
            </c:if>
            <c:if test="${requestScope.page.totalPage>=5}">
        ...
        <c:if test="${requestScope.page.pageNo<requestScope.page.totalPage-1}">
            <li><a href="${requestScope.page.url}&begin=${requestScope.page.pageNo+1}&PageSize=${requestScope.page.pageSize}">下页</a></li>
        </c:if>
        <%--            <li><a href="#">末页</a></li>--%>
        ...
        $("button.add-to-cart").click(function () {
            ...
            if(stock>0){
                ...
                $.getJSON("CartServlet",{
                    ...
                    console.log(data.counts);
                    $("#cart_count").text(data.counts);
                })
            }
        })
        $("#jump").click(function () {
            location.href="http://localhost:8080/jiaju/views/member/cart.jsp";
        })
    })
</script>
</body>
